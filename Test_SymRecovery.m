...
s_fTrainSize = 1000; % Training size
s_fTestSize = 1000; % Test data size
s_nRxAntennas = 1;
...
v_nCurves   = [...          % Curves
    ...
    1 ...                   % SP algorithm
    0 ...                   % SP algorithm - CSI uncertainty
    0 ...                   % Deep Viterbi - perfect CSI
    0 ...                   % Deep Viterbi - CSI uncertainty
    0 ...                   % Viterbi algorithm - perfect CSI
    0 ...                   % Viterbi algorithm - CSI uncertainty
    ];
...
for eIdx=1:length(v_fExps)
    ...
    % m_Rtrain2 = fliplr(m_fNoisyChannel) * m_fStrain;
    ...
    for kk=1:s_fNumFrames
        ...
        noisyChannel = m_fChannel + sqrt(s_fEstErrVar)*randn(size(m_fChannel)).*m_fChannel;
    ...
    for mm=1:length(v_fSigWdB)
        ...
        if(v_nCurves(4)==1)
            m_fNoisyChannel = m_fChannel + sqrt(s_fEstErrVar)*randn(size(m_fChannel))*diag(m_fChannel);
            m_fLikelihood = zeros(s_fTestSize, s_nStates);
            ...
            for ii=1:s_nStates
                ...
                v_fS = 2*(v_fX - 0.5*(s_nConst+1));
                % m_fLikelihood(:,ii) = mvnpdf(bsxfun(@minus,m_fYtest,fliplr(m_fChannel)*v_fS)',zeros(1,2),s_fSigmaW*eye(2));
                m_fLikelihood(:,ii) = normpdf(m_fYtest'-fliplr(m_fNoisyChannel)*v_fS,0,s_fSigmaW);
            end
